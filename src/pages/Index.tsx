...
const Index = () => {
  ...
  const renderScheduleItem = (schedule: Schedule) => {
    return <div key={schedule.id} className="flex items-center p-3 border rounded-md mb-2 hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/escalas/${schedule.id}`)}>
  ...
  const renderMemberItem = (member: Member) => {
    return <div key={member.id} className="flex items-center p-3 border rounded-md mb-2 hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/membros/${member.id}`)}>
  ...
  const renderSongItem = (song: Song) => {
    return <div key={song.id} className="flex items-center p-3 border rounded-md mb-2 hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/repertorio/${song.id}`)}>
  ...
  return <div className="animate-fade-in">
      ...
      <p className="text-gray-600 mb-8">Gestão de Equipes - Igreja Batista Renovada Cristo À Única Esperança</p>
      
      <DashboardStats totalMembers={mockMembers.length} activeMembers={activeMembers.length} upcomingSchedules={getUpcomingSchedules().length} totalSongs={mockSongs.length} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        <Card>
          ...
          <CardContent className="pb-2">
            {upcomingSchedules.length > 0 ? upcomingSchedules.map(renderScheduleItem) : <p className="text-sm text-gray-500 text-center py-4">
        ...
        <Card>
          ...
          <CardContent className="pb-2">
            {mockMembers.length > 0 ? mockMembers.slice(0, 3).map(renderMemberItem) : <p className="text-sm text-gray-500 text-center py-4">
        ...
        <Card>
          ...
          <CardContent className="pb-2">
            {mockSongs.length > 0 ? mockSongs.slice(0, 3).map(renderSongItem) : <p className="text-sm text-gray-500 text-center py-4">
